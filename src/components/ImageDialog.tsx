...
export const ImageDialog = ({open, onOpenChange, imageUrl, altText, title, description} : ImageDialogProps) => {
    ...
    return (
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent variant="image" className="bg-transparent border-0 shadow-none">
                <div className="relative inline-block">
                    ...
                    />
                        ...
                        <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/80 via-black/40 to-transparent p-4 sm:p-6 rounded-b-lg">
                            <DialogTitle className={imageLoaded ? "text-white text-lg sm:text-xl lg:text-2xl xl:text-3xl mb-1 sm:mb-2 title-style" : "sr-only"}>
                                {title}
                            </DialogTitle>
                            {imageLoaded && (
                                <DialogDescription className="text-gray-200 text-xs sm:text-sm lg:text-base leading-relaxed">
                                    {description}
                                </DialogDescription>
                            )}
                        </div>
